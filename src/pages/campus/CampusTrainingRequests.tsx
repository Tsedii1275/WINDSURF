...
import { Search, Eye, Calendar, User, Loader2, CheckCircle, XCircle } from "lucide-react";
...
const TrainingRequests = () => {
  const [requests, setRequests] = useState(mockTrainingRequests);
  const [searchTerm, setSearchTerm] = useState("");
  const [processingId, setProcessingId] = useState<string | null>(null);
  const [showDialog, setShowDialog] = useState(false);
  const [selectedRequest, setSelectedRequest] = useState<typeof mockTrainingRequests[0] | null>(null);
  ...
  const handleView = (request: typeof mockTrainingRequests[0]) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        ...
        description="Review and manage training applications submitted by public users"
      />
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-grow">
              ...
              <Input
                placeholder="Search by applicant name, requested item, or ID..."
                className="pl-10"
                value={searchTerm}
...
interface RequestTableProps {
  requests: typeof mockTrainingRequests;
  processingId: string | null;
  onApprove: (id: string) => void;
  onReject: (id: string) => void;
  getStatusBadge: (status: string) => React.ReactNode;
  onView: (request: typeof mockTrainingRequests[0]) => void;
}
        ...
        <tbody>
          {requests.map((request) => (
            <tr key={request.id} className="bg-white border-b hover:bg-gray-50 transition-colors">
              ...
              <td className="px-6 py-4 font-medium">{request.requestedItem}</td>
              <td className="px-6 py-4">
...
// Training Request Details Dialog Component
interface TrainingRequestDetailsProps {
  request: typeof mockTrainingRequests[0];
  onApprove: (id: string) => void;
  onReject: (id: string) => void;
  onClose: () => void;
}

const TrainingRequestDetails = ({ request, onApprove, onReject, onClose }: TrainingRequestDetailsProps) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-3">
            <div>
              <label className="text-xs font-medium text-gray-500">Requested Training</label>
      ...
      <div className="space-y-4">
        <h4 className="font-semibold text-gray-800">Motivation & Reason</h4>
